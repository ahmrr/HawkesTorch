...
def state_mult(B: torch.Tensor, A: torch.Tensor):
    """
    Broadcastable, optimized matrix multiplication (right-multiply) for augmented state matrices of a specific structure.
    ...
    result = torch.empty_like(A)  # Shape: [R, n2, K, M+1]
...
def prefix_scan(x, prefix_func, dim=0, pad_value=0):
    """
    Obtained from https://github.com/glassroom/torch_parallel_scan/blob/main/torch_parallel_scan/torch_parallel_scan.py
...
def reduce_scan(x, reduce_func, dim=0):
    """
    Obtained from https://github.com/glassroom/torch_parallel_scan/blob/main/torch_parallel_scan/torch_parallel_scan.py
