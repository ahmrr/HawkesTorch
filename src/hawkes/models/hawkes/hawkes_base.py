...
from ._hawkes_nll import HawkesLogSumIntensity, HawkesIntegratedExcitation


@dataclass
class HawkesPenalty:
    """
    Penalization hyperparameters for the Hawkes process.

    Attributes:
        alpha: Penalty for the excitation parameters, or None.
        gamma: Penalty for the decay parameters, or None.
        penalize_alpha_diag: Whether to apply penalties to diagonal entries of alpha.
    """

    alpha: Penalty | None = None
    gamma: Penalty | None = None
...
class HawkesBase(torch.nn.Module, ABC):
    ...
    Concrete subclasses must provide properties for alpha, and gamma
    ...
    def __init__(
        ...
        if runtime_config.deterministic_sim:
            # Seed RNGs for reproducible simulations
    ...
    def state_dict(self, *args, **kwargs):
        ...
        with torch.no_grad():
            state_dict["mu"] = self.base_process.get_save_data()
    ...
    def mu_at_events(self, t: torch.Tensor, m: torch.Tensor) -> torch.Tensor:
        """Base intensity values at event times of shape [N]."""
    ...
    def gamma(self) -> torch.Tensor:
        """Decay memory vector of shape [K]."""
        pass
    ...
    def penalty(self) -> float:
        ...
        if not self.penalization.penalize_alpha_diag:
            off_diagonal_mask = ~torch.eye(
                self.M, dtype=torch.bool, device=self.device
            )[None, ...]
            alpha = torch.where(off_diagonal_mask, alpha, torch.zeros_like(alpha))

        if self.penalization.alpha is not None:
            penalty += self.penalization.alpha(alpha)
        if self.penalization.gamma is not None:
            penalty += self.penalization.gamma(gamma)
